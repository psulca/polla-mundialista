...
import { getLeaderboard, getRoundLeaderboard } from "@/lib/data/ranking";
...
export default async function RankingPage({
  ...
  const [rows, me] = await Promise.all([
    fechaRound ? getRoundLeaderboard(fechaRound.id, "general") : getLeaderboard("general"),
    getCurrentPlayer(),
  ]);
    ...
    <div className="flex h-full flex-col">
      ...
      <div className="mx-auto w-full max-w-md shrink-0 px-4 pb-3 pt-7">
        <div className="flex items-center justify-between">
          ...
          {me && rows.some((r) => r.playerId === me.id) && <JumpToMe playerId={me.id} />}
        </div>
        ...
        {pot != null ? (
          <div className="mt-3 flex items-center justify-between rounded-xl border border-gold/30 bg-gold/10 px-4 py-2 text-sm">
      ...
      <div className="mx-auto w-full min-h-0 max-w-md flex-1 px-4 pb-4">
        <ScrollArea className="ring-sticker h-full overflow-hidden rounded-2xl border border-border bg-card">
          {rows.length === 0 && (
